...
__simt_vf__ LAUNCH_BOUND(VF_MAX_THREAD_NUM) __aicore__ void FftC2RStockhamForwardStage(
    ...
    __gm__ float * __restrict__ fftMat,
    ...
    int32_t radixListLen)
{
    ...
    for (int64_t outIdx = tid; outIdx < totalOutputComplex; outIdx += stride) {
        ...
        for (int32_t v = 0; v < radix; v++) {
            float wrRe = fftMat[(2 * u) * radix + v];
            float wrIm = fftMat[(2 * u + 1) * radix + v];
...
public:
    ...
    __aicore__ inline void Init(
        __gm__ float * __restrict__ gm_input,
        __gm__ float * __restrict__ gm_fft_matrix_array,
...
private:
    ...
    __gm__ float * __restrict__ gm_fft_matrix_array_;
    ...
    __gm__ int64_t * __restrict__ gm_fft_offset_arr_;
    ...
    __gm__ uint8_t * __restrict__ gm_tiling_para)
{
    ...
    gm_fft_offset_arr_ = (__gm__ int64_t *)((__gm__ uint8_t *)tiling_buf + baseOffset + MAX_FFT_STAGES * 4 + MAX_FFT_STAGES * 8);
    ...
    gm_fft_matrix_array_ = gm_fft_matrix_array;
...
__aicore__ inline void FftC2RKernelMultiCore::Process()
{
    ...
    for (int32_t step = 0; step < radixListLen_; step++) {
        ...
        __gm__ float * __restrict__ fftMat = gm_fft_matrix_array_ + gm_fft_offset_arr_[step];
    ...
    __gm__ uint8_t * __restrict__ gm_tiling_para)
{
    ...
    kernel.Init(gm_input, gm_fft_matrix_array, gm_tw_matrix_array, radix_list,
