            ...
            for (int k_part_idx = 0; k_part_idx < k_part_loop; k_part_idx++) {
                ...
                if (M != 1 && m_actual == 1 && trans_a) {
                    AscendC::MmadParams mmParam = AscendC::MmadParams(
                        ...
                        k0_actual,          // kIn
                        0,                  // unitFlagIn
                        false,              // cmatrixSourceIn
                        init_c              // cmatrixInitValIn
                    );
...
// ================= 新增：Host 侧封装函数 =================
// 因为这个函数本身是在 Host 侧执行的
void dft_r2c_kernel_do(
    uint32_t blockDim, 
    void* l2ctrl, 
    void* stream, 
    GM_ADDR gmA, 
...
{
    // 在这里使用 <<<>>> 语法调用上面的核函数
    dft_r2c<<<blockDim, l2ctrl, stream>>>(gmA, gmB, gmC, workSpace, tilingParaGm);
}