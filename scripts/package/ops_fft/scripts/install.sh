...
# ----------------------------------------------------------------------------
# ops-fft Installation Script (Built-in Package)
# ----------------------------------------------------------------------------
...
init_env() {
  ...
  logandprint "[INFO]: Execute the ops_fft run package."
  ...
  if [ "${installed_version}" = "" ]; then
      logandprint "[INFO]: Version of installing ops_fft module is ${RUN_PKG_VERSION}."
  else
      logandprint "[INFO]: Existed ops_fft module version is ${installed_version}, the new version is ${RUN_PKG_VERSION}."
  fi
}
...
check_pre_install() {
  ...
  if [ "${IS_UPGRADE}" = "y" ]; then
      if [ ! -e "${INSTALL_INFO_FILE}" ]; then
          logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${TARGET_INSTALL_PATH} has no ops_fft installed, upgrade failed."
          exitlog
          exit 1
      fi
...
clean_before_reinstall() {
  ...
  if [ -z "${existed_files}" ]; then
      logandprint "[INFO]: Directory is empty, directly install ops_fft module."
      return 0
  fi

  if [ "${IS_QUIET}" = "y" ]; then
      logandprint "[WARNING]: Directory has files existed or ops_fft installed, continuing installation."
  else
      if [ ! -f "${INSTALL_INFO_FILE}" ]; then
          logandprint "[INFO]: Directory has files existed, do you want to continue? [y/n]"
      else
          logandprint "[INFO]: ops_fft package has been installed at $(get_installed_info "${KEY_INSTALLED_PATH}"), version $(get_installed_info "${KEY_INSTALLED_VERSION}"), this package version is ${RUN_PKG_VERSION}, do you want to continue? [y/n]"
      fi
      while true; do
          read yn
          if [ "$yn" = "n" ]; then
  ...
  if [ "${installed_path}" = "${TARGET_VERSION_DIR}" ]; then
      logandprint "[INFO]: Clean the installed ops_fft module before install."
...
install_package() {
  ...
  logandprint "[INFO]: ============================================"
  logandprint "[INFO]: Starting ops_fft installation..."
  ...
  logandprint "[INFO]:"
  logandprint "[INFO]: To use ops-fft, you may need to set environment:"
...
uninstall_package() {
  ...
  # Check if ops_fft is installed by looking for install info file
  if [ ! -f "${INSTALL_INFO_FILE}" ]; then
      logandprint "[INFO]: ops_fft is not installed (no install info found at ${INSTALL_INFO_FILE})"
      exitlog
      exit 0
  fi
