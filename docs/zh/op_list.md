# 算子列表

> 说明：
> - **算子目录**：目录名为算子名小写下划线形式，每个目录承载该算子所有交付件，包括代码实现、文档等，目录介绍参见[项目目录](./context/dir_structure.md)。
> - **算子执行硬件单元**：项目中提到的算子指AI Core算子。

项目提供的所有算子分类和算子列表如下：

| 算子分类 | 算子目录 | op_kernel | op_host | 算子执行硬件单元 | 说明 |
| :--- | :--- | :---: | :---: | :--- | :--- |
| fft | [fft1_d](#fft1_d) | √ | √ | AI Core | 一维复数快速傅里叶变换，支持FP32数据类型。 |
| fft | [fft2_d](#fft2_d) | √ | √ | AI Core | 二维复数快速傅里叶变换，支持FP32数据类型。 |
| fft | [irfft1_d](#irfft1_d) | √ | √ | AI Core | 一维复数到实数逆傅里叶变换，支持FP32数据类型。 |
| fft | [rfft1_d](#rfft1_d) | √ | √ | AI Core | 一维实数快速傅里叶变换，支持FP32数据类型。 |

## 算子详细说明

### fft1_d

对一维复数序列执行快速傅里叶变换（FFT）。

**目录位置**：`src/fft1_d/`

**接口文档**：[FFT_1D（aclfftExecC2C）](./API%20Reference/FFT_1D.md)

**支持的芯片**：Ascend 950、Ascend 910B

**支持的数据类型**：FP32

**输入**：
- x：输入张量（复数序列）

**输出**：
- y：输出张量（FFT结果，复数序列）

### fft2_d

对二维复数序列执行快速傅里叶变换（FFT）。

**目录位置**：`src/fft2_d/`

**接口文档**：[FFT_2D（aclfftPlan2d/aclfftExecC2C）](./API%20Reference/FFT_2D.md)

**支持的芯片**：Ascend 950、Ascend 910B

**支持的数据类型**：FP32

**输入**：
- x：输入张量（复数序列）

**输出**：
- y：输出张量（FFT结果，复数序列）

### irfft1_d

对一维复数序列执行逆快速傅里叶变换（IFFT），输出实数序列。

**目录位置**：`src/irfft1_d/`

**接口文档**：[FFT_1D（aclfftExecC2R）](./API%20Reference/FFT_1D.md)

**支持的芯片**：Ascend 950、Ascend 910B

**支持的数据类型**：FP32

**输入**：
- x：输入张量（复数序列）

**输出**：
- y：输出张量（IFFT结果，实数序列）

### rfft1_d

对一维实数序列执行快速傅里叶变换（FFT）。

**目录位置**：`src/rfft1_d/`

**接口文档**：[FFT_1D（aclfftExecR2C）](./API%20Reference/FFT_1D.md)

**支持的芯片**：Ascend 950、Ascend 910B

**支持的数据类型**：FP32

**输入**：
- x：输入张量（实数序列）

**输出**：
- y：输出张量（FFT结果，复数序列）

## 算子分类说明

### fft 类算子

ops-fft是[CANN](https://hiascend.com/software/cann)（Compute Architecture for Neural Networks）算子库中提供的**FFT变换算子库**，主要提供以下功能：

- **fft1_d**：一维复数FFT运算，对复数序列执行快速傅里叶变换。
- **fft2_d**：二维复数FFT运算，对二维复数序列执行快速傅里叶变换。
- **irfft1_d**：一维复数到实数IFFT运算，对复数序列执行逆傅里叶变换输出实数。
- **rfft1_d**：一维实数FFT运算，对实数序列执行快速傅里叶变换。

## 支持的芯片版本

| 芯片类型 | 支持状态 |
| :--- | :---: |
| Ascend 950PR | √ |
| Ascend 950DT | √ |
| Ascend 910B | √ |

> **说明**：本项目当前支持 Ascend 950 系列和 Ascend 910B 系列芯片，其他芯片版本的支持情况请关注后续更新。

## 快速开始

如需快速体验算子调用，请参考：
- [环境部署](./context/quick_install.md)：搭建基础环境
- [算子调用](./invocation/quick_op_invocation.md)：编译部署并调用算子
- [接口参考](./API%20Reference/README.md)：`aclfft` 系列 C 接口说明
